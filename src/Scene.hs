...
getSceneConfig path = do
  let pathAux = fromMaybe "" path
  if pathAux == "" then return $ Success defaultSceneConfig
  else do
    result <- loadSceneConfig pathAux
    case result of
      Error err -> 
        case err of 
          FileNotFound -> return $ Error FileNotFound 
          _ -> return $ Success defaultSceneConfig          
...
loadSceneConfig path = do
  ...
  if not exists
    then return $ Error FileNotFound
      ...
      case runParser sceneParser contents of
